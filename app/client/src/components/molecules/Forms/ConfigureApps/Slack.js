    ...
    const handleConfigure = async (e) => {
      ...
      if (channel && token) {
        try {
          ...
          const response = await instance.post('/api/slack', {token, channel},
          {
            headers: {
              'Authorization': `Bearer ${authToken}`,
              }
          });
          if(response.status === 200 && response.data.message === "Slack bot token and channel stored successfully"){
